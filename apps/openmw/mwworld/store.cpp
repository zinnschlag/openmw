...
namespace MWWorld 
{
    /// Merges loading cell references and replaces old cell if necessary  
    ///
    /// \param esm
    /// \param cell id
    void Store<ESM::Cell>::load( ESM::ESMReader &esm, const std::string &id )
    {
        ...
        //  are not available until both cells have been loaded! The following parts are followed to load a cell:
        //  1) pre load esm and move cell references to new cell
        //  2) merge cell references with new cell, replace old cell if one exists 
        ...
        while (esm.isNextSub("MVRF")) 
        {
            ...
            MWWorld::Store<ESM::Cell> & cStore;
            cStore = const_cast<  MWWorld::Store<ESM::Cell> & >( mEsmStore->get<ESM::Cell>() );
            
            ESM::Cell * cellAlt;
            cellAlt = const_cast<ESM::Cell*>( cStore.searchOrCreate(cMRef.mTarget[0], cMRef.mTarget[1]) );

            // Get regular moved reference data. Adapted from CellStore::loadRefs. Maybe we can optimize the following
            //  implementation when the other implementation works as well.
            bool deleted = false;
            ...
            ESM::CellRefTracker::iterator iter;
            iter = std::find( cellAlt->mLeasedRefs.begin(), cellAlt->mLeasedRefs.end(), ref.mRefNum );
