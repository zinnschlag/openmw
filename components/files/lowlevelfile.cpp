...
void LowLevelFile::open(const char *filename)
{
    assert(mHandle == NULL);
...
void LowLevelFile::open(const char *filename)
{
    ...
    wfile = new wchar_t[len+1];
    MultiByteToWideChar(CP_UTF8, 0, filename, -1, wfile, len);

    HANDLE handle = CreateFileW(wfile, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, 0, 0);
    delete[] wfile;
